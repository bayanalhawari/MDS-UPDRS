/**
 * Builds the MDS-UPDRS composition as openEHR "canonical" XML — exactly in
 * the format this EHRbase server has been confirmed to accept (see the
 * example the user provided from the template-example endpoint).
 *
 * We deliberately avoid the FLAT format: EHRbase couldn't resolve a single
 * path with it ("Could not consume Parts [...]" for EVERY key sent), which
 * points to a mismatch between the FLAT path scheme built here and the
 * template actually deployed on the server. Canonical XML is more robust
 * because it follows the RM structure 1:1.
 *
 * Part I, II and IV are not sent: per the web template, their
 * SECTION/OBSERVATION nodes are optional (min=0) and don't contain any
 * fillable fields anyway — they exist only as empty tabs in the UI until
 * the template is extended for them.
 */

// Fixed markers, known from the web template, for the two ITEM_TREEs
// inside the Part III event (see src/lib/template.json).
...
function normalizeTime(value) {
  // <input type="time"> liefert "HH:MM" — openEHR DV_TIME will "HH:MM:SS".
...
// "Last levodopa dose" (at0241) is collected as plain elapsed minutes via
// the MinutesWidget (see FormField.jsx). The archetype's RM type is now
// DV_DURATION, so we convert the minute count into an ISO 8601 duration
// string ("PT90M") here. Unlike the previous DV_TIME workaround, no 24h
// wraparound is needed — a duration can represent any number of minutes.
function normalizeDuration(value) {
  if (typeof value === 'number' || (typeof value === 'string' && /^\d+$/.test(value))) {
    return `PT${Number(value)}M`;
  }
  // already an ISO 8601 duration string (e.g. pasted/edited value)
  return value;
}

/** Rendert ein einzelnes ELEMENT (Blatt) als XML, je nach RM-Typ. */
 ...
 * und, falls vorhanden, nach ihrem CLUSTER-Elternknoten — und rendert alles
 * als XML.
 */
function buildPart3Items(fields, values) {
  const dataItems = [];
  const stateItems = [];
  const clusters = new Map(); // atCode -> { name, children: [] }
  ...
  for (const field of fields) {
    ...
    if (field.aqlPath.includes(DATA_TREE_MARKER)) {
      ...
      continue; // not a Part III data/state field (shouldn't happen here)
    }
 ...
 * @param {object[]} opts.fields - aus parseTemplate(template).fields
 * @param {object} opts.values - { [aqlPath]: value } aus dem Formular
 * @param {string} [opts.composerName] - Name des Untersuchers/der Untersucherin (Examiner)
 * @param {string} [opts.subjectName] - Name des Patienten
 * @param {string} [opts.startTime] - ISO-Zeitstempel des Assessments (Assessment Date)
 * @param {string} [opts.location] - Ort der Untersuchung (Pflichtfeld im Template, context/location)
 * @param {string} [opts.siteId] - Site ID (optionales Feld, context/other_context/items[at0036])
